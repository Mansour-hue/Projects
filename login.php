...
<nav id="topnav" class="topnav">
 <img class="brand" src="image/logo.png">
  ...
  ?>
   ...
   <a href="login.php?s=of">تسجيل الخروج</a>
   <?php
   }
   else
   {
 ...
 <div class="container">
    ...
    <div class="row-container">
        ...
        <div class="col2" textarea="center">
            <a href="signup.php"><h1>انشاء حساب</h1></a>
			<h1>سجل دخولك</h1>
        </div>
 ...
 if(isset($_POST['send']))
 {
   ...
   else
   {
     if(strlen($_POST['password'])>20)
         {
           $errors['password2']="يرجى ادخال اقل من 20 حروف";
         }
      else
      {
   ...
   if($_POST['password'] !== $_POST['password'])
      ...
      {
        $password2= cleanData($_POST['password']);
      }
...
if(isset($_POST['send']))
{
	...
	$q="SELECT * FROM `user` WHERE `email`='{$_POST['email']}' or `password`='{$pass}'";
	...
	while($row=mysqli_fetch_assoc($r))
	{
		if ($_POST['email']==$row['email'])
		{
			...
			if($pass==$row['password'])
			{
				if ($row['active']==1)
				{
						$_SESSION['USERNAME'] = $row['user_name'].' '.$row['last_user_name'];
				...
				{
					echo "<h1> الرجاء الانتظار حتى يتم تفعيل المستخدم الرجاء المحاوله في وقت احر</h1>";
				}
			...
			{
				echo"your user pass is flase";
			}
		}
		else
		{
			echo"your user name is flase";
		}
